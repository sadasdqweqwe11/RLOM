...
public class HLXBExcel extends ExcelModel {
	...
	public HLXBExcel(List<RLOrder> rlOrders) {
	...
	private void generateExcel(List<RLOrder> rlOrders) {
		if (null != rlOrders && rlOrders.size() != 0) {
			...
			this.data = new Object[HLXBExcel.COLUMN_NUMBER][rowCount];
			int rowNumber = 0;
			for (RLOrder p : rlOrders) {
				...
				this.data[7][rowNumber] = p.getShipstate();
